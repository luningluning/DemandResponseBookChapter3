...
% Example 3.3 - produce figure 3.7 (a) and (b)
...
plot(Time,Troom_record,color(1),'LineWidth',1)
% plot(Tmass_record,'b')
xlabel('Time (minute)')
ylabel('Temperature (^oC)')
hold on
title('(a) SS-RCQ versus Linearized ETP')
xlim([0 60*60*hours]/60)
...
set(gcf,'DefaultAxesFontSize',14)  %<--------set character size
set(gcf,'DefaultTextFontSize',14)
plot(Time,Troom_record_simple,'b','LineWidth',1)
% plot(Tmass_record,'b')
xlabel('Time (minute)')
ylabel('Temperature (^oC)')
...
set(gcf,'DefaultAxesFontSize',14)  %<--------set character size
set(gcf,'DefaultTextFontSize',14)
plot(Time,Troom_record_actual,'r','LineWidth',1)
hold on
plot(Time,Troom_record_simple,'b','LineWidth',1)
% plot(Tmass_record,'b')
xlabel('Time (minute)')
...
title('(b) Linearized ETP with no Error Correction')
legend('SS-RCQ','Linear ETP')
xlim([0 60*60*hours]/60)
ylim([19 22])
